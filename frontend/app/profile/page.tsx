...
function ProviderIcon({ provider }: { provider: AuthAccount["provider"] }) {
  if (provider === "GITHUB") {
    return (
    ...
    <div className="min-h-screen bg-bg text-ink font-sans antialiased overflow-x-hidden">
      <div className="mx-auto max-w-3xl px-5 sm:px-8 pt-28 pb-24">
        ...
        <div className="mt-6 grid gap-6 sm:grid-cols-2">
          {/* Linked accounts */}
          <section className="rounded-3xl border border-line bg-surface shadow-soft p-6">
            <h2 className="text-sm font-semibold tracking-tight">
              Linked accounts
            </h2>
            <p className="mt-1 text-xs text-ink-3">
              Ways you can sign in to CodeClash.
            </p>

            <ul className="mt-5 space-y-3">
              {accounts.length === 0 && (
                <li className="text-sm text-ink-3">No linked accounts.</li>
              )}
                  ...
                  <span className="flex-1 min-w-0">
                    <span className="block text-sm font-medium">
                      {account.provider === "LOCAL"
                        ? "Email & password"
                        : account.provider.charAt(0) + account.provider.slice(1).toLowerCase()}
                    </span>
          ...
          <section className="rounded-3xl border border-line bg-surface shadow-soft p-6">
            ...
            <div className="mt-5 flex items-center gap-1.5 text-xs text-ink-3">
              ...
              <span>
                This page shows {sessions.length} of your active sessions.
              </span>
            </div>
          </section>
        </div>
